...
main() {
    ...
    for ((packet_size=PKT_SMALLEST_SIZE; packet_size<=PKT_BIGEST_SIZE; packet_size+=PKT_STEP)); do
        for queues_size in $(seq 1 $MAX_QUEUES); do
            ...
            SOURCE_DIR="${WORKSPACE}/results-acm-conext/${packet_size}Bytes/tx_rx_queue_${QUEUE_SIZE}"
            ...
            for i in $(seq 1 $TEST_ITERATIONS); do
                ...
                "${SCRIPTS}/acm_conext_start_tests_udp.sh" "$SOURCE_DIR" "$i" "$QUEUE_SIZE" "$TEST_DURATION" "$packet_size" > output.log 2>&1 &
