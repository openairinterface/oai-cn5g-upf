...
send_keys_trex() {
  ...
  tmux send-keys -t "$session_name:0.0" ""${passwd}"" C-m
...
send_keys_upf_logs() {
  ...
  # tmux send-keys -t "$session_name:1.0" "sudo ./xdpdump -i enp5s0f1 --use-pcap -w "$1"/capture_"$2"_enp5s0f1.pcap" C-m
  ...
  # tmux send-keys -t "$session_name:1.1" "sudo ./xdpdump -i enp5s0f0 -w "$1"/capture_"$2"_enp5s0f0.pcap" C-m
  ...
  tmux send-keys -t "$session_name:1.2" "sudo ./xdp-monitor -e >> "$1"/xdp-monitor_"$2".log" C-m "{passwd}" C-m
  ...
  tmux send-keys -t "$session_name:1.4" "python3 run_udp.py -m 100% -p 1 -f udp -q "$3" -d "$4" -s "$5"" C-m "{passwd}" C-m
