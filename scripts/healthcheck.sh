...
check_port_status() {
  local interface_name=$1
  local port=$2
  local ip_interface=$(ifconfig $interface_name | grep inet | grep -v inet6  | awk '{print $2}')
  if [[ -z "$ip_interface" ]]; then
    echo "Error: Could not retrieve IP address for interface $interface_name."
    return 1
  fi

  # Check if the port is listening on the specified IP or on all interfaces (0.0.0.0).
  # Use ss instead of netstat -p: the -p flag reads /proc/<pid>/fd symlinks which
  # can fail transiently and cause false negatives.
  if ss -uln | grep -qE "(${ip_interface}|0\.0\.0\.0|\*):${port}[^0-9]"; then
