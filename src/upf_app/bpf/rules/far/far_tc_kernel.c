...
int handle_broadcast(struct __sk_buff *skb)
{
    ...
    struct callback_ctx callback_ctx = { .skb = skb, .ifindex = ifindex, .size = 0 };
    ...
    // Broadcast should be sent to all existing PDU session. Use m_next_rule_eth_prog_index map
    long n = bpf_for_each_map_elem(&m_next_rule_eth_prog_index, broadcast_callback_fn, &callback_ctx, 0);
    action = TC_ACT_SHOT;
