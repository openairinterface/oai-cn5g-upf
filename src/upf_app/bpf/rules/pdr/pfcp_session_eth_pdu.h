...
static __always_inline u32 handle_far__uplink(
  ...
  if (!session) {
    bpf_debug("ETH DPU: No next prog found for TEID %u, source_value %u, qfi %u", teid, source_value, qfi);
    return XDP_DROP;
  }
...
handle_downlink_traffic__eth_pdu(struct xdp_md* ctx) {
  ...
  if (pdu_session) {
    bpf_debug("ETH PDU: Found PDU session in MAC learning table. DL TEID: %u", pdu_session->teid);
