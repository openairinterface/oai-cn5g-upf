...
//   // clang-format off
//   if(p_pdr->outer_header_removal.outer_header_removal_description !=
...
//         p_pdr->pdi.source_interface.interface_value); bpf_debug("TEID: %d",
//         teid); bpf_debug("TEID: %d", p_pdr->pdi.fteid.teid);
...
//       teid, source_value, ipv4_address);
...
//              \t\t\t\t\t\t 2. Endianess problem!\n \
//              \t\t\t\t\t\t Map and Key values are saved in different
//              endianess!\n\ \t\t\t\t\t\t Map Hash Key and Key not
//              matching\n");
...
//  * - The TEID from GTP GPDU with the TEID stored in PDR.
//  * - Source IP from IP header with source address stored in PDI.
//  * - Interface from PDI with ACCESS or CORE interface (it depends on if it is
//  UL
