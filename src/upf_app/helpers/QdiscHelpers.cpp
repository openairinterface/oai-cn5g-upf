...
struct nl_sock* QdiscHelper::createSocket() {
  ...
  if (!(socket = nl_socket_alloc())) {
    Logger::upf_app().error(
        "nl_socket_alloc: Unable to allocate netlink socket");
    return nullptr;
  }
  ...
  if ((err = nl_connect(socket, NETLINK_ROUTE)) < 0) {
    Logger::upf_app().error(
        "nl_connect:Unable to connect to the netlink socket");
...
struct nl_cache* QdiscHelper::createLinkCache(struct nl_sock* socket) {
  ...
  if ((err = rtnl_link_alloc_cache(socket, AF_UNSPEC, &linkCache)) < 0) {
    Logger::upf_app().error(
        "Unable to allocate link cache: %s\n", nl_geterror(err));
    nl_socket_free(socket);
    return nullptr;
  }
...
struct rtnl_link* QdiscHelper::createLink(
    const char* iface, struct nl_cache* linkCache, struct nl_sock* socket) {
  struct rtnl_link* link;
  Logger::upf_app().info("Create a Netlink Link object");

  if (!(link = rtnl_link_get_by_name(linkCache, iface))) {
    Logger::upf_app().error(
        "rtnl_link_get_by_name: Interface %s not found\n", iface);
    nl_socket_free(socket);
    return nullptr;
  }
...
int QdiscHelper::configureRootQdisc(
    struct nl_sock* socket, struct rtnl_link* link, struct rtnl_qdisc* qdisc,
  ...
  if ((err = rtnl_tc_set_kind(TC_CAST(qdisc), qdiscAtt->scheduler))) {
    Logger::upf_app().error(
        "rtnl_tc_set_kind: Cannot allocate :%s\n", qdiscAtt->scheduler);
    return (err);
  }
  ...
  Logger::upf_app().info(
      "Submit Qdisc Creation Request to Kernel and Wait for Response");
...
int QdiscHelper::configureRootClass(
    struct nl_sock* socket, struct rtnl_link* link,
    struct rtnl_class* qdiscClass, classParams* classAtt) {
  int err;
  ...
  if ((err = rtnl_tc_set_kind(TC_CAST(qdiscClass), classAtt->scheduler))) {
    Logger::upf_app().error(
        "Unable to set Scheduler %s to Class", classAtt->scheduler);
    return (err);
  }
...
*/
  ...
  /* Submit request to kernel and wait for response */
  Logger::upf_app().info(
      "Submit Class Creation Request to Kernel and Wait for Response");
...
void QdiscHelper::releaseNetlinkQdisc(
    struct nl_sock* socket, struct rtnl_qdisc* qdisc) {
...
int QdiscHelper::configureParentClass(
    struct nl_sock* socket, struct rtnl_link* link,
    struct rtnl_class* parentClass, struct classParams* classAtt,
  ...
  if ((err = rtnl_tc_set_kind(TC_CAST(parentClass), classAtt->scheduler))) {
    Logger::upf_app().error(
        "Unable to set Scheduler %s to Parent Class", classAtt->scheduler);
    return (err);
  }
...
int QdiscHelper::configureLeafClass(
  ...
  if ((err = rtnl_tc_set_kind(TC_CAST(leafClass), classAtt->scheduler))) {
    Logger::upf_app().error(
        "Unable to set Scheduler %s to Leaf Class", classAtt->scheduler);
    return (err);
  }
