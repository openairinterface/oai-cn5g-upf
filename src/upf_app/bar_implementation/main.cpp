...
#include "sbi/include/Sqlite3Tables.hpp"
#include <iostream>
#include <pcap.h>
...
int main() {
    ...
    const char* filterExpr = "icmp"; // Optional filter expression


    PacketCapture packetCapture;
    if (packetCapture.captureTraffic(interface, packetCount, delay, filterExpr) == 0) {
        std::cout << "Packet capture completed successfully." << std::endl;
    } else {
        std::cerr << "Packet capture failed." << std::endl;
    }
    ...
    // int delay = 30; 
    // PacketCapture packetCapture;
    // packetCapture.capturePacket(delay, *sqlite3Helper, pcapDumper);
