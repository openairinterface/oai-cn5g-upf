...
TEST_F(FramedRoutingTest, AddRoutes) {
  const uint32_t gatewayip = 0x01010102;
  auto framed_route        = pfcp::framed_route_s{
      "192.168.2.0/24 192.168.1.0/24 12.1.1.0/25 192.168.1.0/25 130.0.0.5 192.168.128.0"};
...
TEST_F(FramedRoutingTest, AddRoutesEmptyIP) {
  auto framed_route = pfcp::framed_route_s{""};
  FramedRoutingTest::fr->addFramedRoute(ue_ip_one, framed_route, 0);
  EXPECT_NO_FATAL_FAILURE();
}
