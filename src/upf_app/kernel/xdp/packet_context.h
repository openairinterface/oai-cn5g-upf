 ...
 * Date:        2026-03
 * Changes:     V17.10.0 struct addition — pdr_mar_id field uncommented:
 *                - pfcp_pdr.h now carries mar_id (__u16, §8.2.123) so the
 *                  PDR→MAR chain can be followed in the XDP pipeline.  The
 *                  matched PDR's mar_id is propagated here so mar_apply.c
 *                  can select the correct MAR rule for multi-access sessions.
 *              Boy Scout cleanup:
 *                - Fixed RULE_MAR_ENABLED §-ref in RULE_* comment block:
 *                  §8.2.74-76 (old, pre-R17 numbering) →
 *                  §8.2.123/124/125 (MAR ID / Steering Functionality /
 *                  Steering Mode per V17.10.0).
 *                - Fixed MAR override comment: §8.2.74 → §8.2.123.
 * 3GPP Refs:   3GPP TS 29.244 V17.10.0 (Release 17, 2024-04) — PFCP Protocol
 *              §8.2.123  MAR ID            §8.2.124  Steering Functionality
 *              §8.2.125  Steering Mode
 */
// clang-format on

/**
 * @file packet_context.h
 * @brief Per-packet processing context shared across BPF tail call chain
 *
 * This structure is the shared scratchpad between tail call programs,
 * stored in a BPF_MAP_TYPE_PERCPU_ARRAY (1 entry, index 0). Each program
 * in the processing chain reads/writes fields relevant to its stage.
 *
 * Population order through the chain:
 *   Entry point     → session_type, ue_ip, qfi, pkt_teid, pkt_filter
 *   Session lookup  → seid, teid_ul, teid_dl, rules_enabled
 *   PDR match       → pdr_id      (TS 29.244 §8.2.21 - PDR ID)
 *   FAR apply       → final_action (TS 29.244 §8.2.26 - Apply Action)
 *   QER apply       → may override final_action (TS 29.244 §8.2.41)
 *   URR apply       → volume/time measurement (TS 29.244 §8.2.44-48)
 *   BAR apply       → (future) buffering (TS 29.244 §8.2.49-50)
 *   MAR apply       → may override redirect target (TS 29.244 §8.2.74-76)
 *
 * @see 3GPP TS 29.244 - PFCP Protocol
 * @see 3GPP TS 23.501 - 5G System Architecture (5.6.10 - PDU Session Types (IP
 * vs Ethernet))
