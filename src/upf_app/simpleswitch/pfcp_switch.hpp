 ...
 *              separator lines, private/public section grouping.
 *              V17.10.0 harmonisation:
 *                - Version bump V17.6.0 → V17.10.0.
 *                - Fixed §7.5.5 → §7.5.6 in handle_pfcp_session_deletion_request
 *                  Doxygen (§7.5.5 = Modification Response in V17.10.0;
 *                  §7.5.6 = Deletion Request).
 *                - Confirmed correct §-refs (no change needed):
 *                    §8.2.37 F-SEID  (cp_fseid2pfcp_sessions comment)
 *                    §8.2.3  F-TEID  (ul_n3_teid2pfcp_pdr + generate_fteid_n3)
 *                    §8.2.62 UE IP Address (ue_ipv4_hbo2pfcp_pdr comment)
 *                    §7.5.2  Session Establishment Request
 *                    §7.5.4  Session Modification Request
 *                - Added ///<  doc-comments to all previously undocumented
 *                  private member variables (recv_buffer_alloc_, recv_buffer_,
 *                  prThreadToCancel, socks_r_ptr, terminatePW_, pdn_if_index,
 *                  seid_generator_, teid_n3_generator__, local_routing, fr,
 *                  timer_max/min_commit_interval_id).
 *                - Added Doxygen to all previously undocumented private methods:
 *                  timer start/stop helpers, commit_changes(), setup(),
 *                  setup_pdn_interfaces(), create_pdn_socket() overloads,
 *                  tun_open(), add_pfcp_session_by_cp/up_seid(),
 *                  remove_pfcp_session(shared_ptr).
 *                - Added Doxygen to undocumented public methods:
 *                  time_out_min/max_commit_interval(),
 *                  remove_pfcp_ul/dl_pdrs_by_up_teid/ue_ip().
...
/*! \file pfcp_switch.hpp
   ...
   Singleton that manages all active PFCP sessions and performs per-packet
   PDR look-up for both uplink (N3) and downlink (N6/tun) data paths.

   Session state is stored in two hash maps (3GPP TS 29.244 V17.10.0):
     cp_fseid2pfcp_sessions — indexed by CP F-SEID (§8.2.37)
     up_seid2pfcp_sessions  — indexed by UP SEID (allocated locally)

   Per-packet look-up tables:
     ul_n3_teid2pfcp_pdr   — uplink:   GTP-U TEID → PDR vector, sorted by
                             precedence (§8.2.11); keyed by F-TEID (§8.2.3)
     ue_ipv4_hbo2pfcp_pdr  — downlink: UE IPv4 (host byte order) → PDR vector;
                             keyed by UE IP Address (§8.2.62)
*/
