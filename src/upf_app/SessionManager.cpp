...
void SessionManager::createSession(std::shared_ptr<SessionBpf> pSession)
{
  ...
  Logger::upf_app().debug("Session %d has been cretead", pSession->getSeid());
}
...
void SessionManager::createBPFSession(std::shared_ptr<pfcp::pfcp_session> pSession)
{
  ...
  Logger::upf_app().debug("Session %d received... Lets prepare the UP", pSession->get_up_seid());
  ...
  Logger::upf_app().debug("PDI extracted from PDR %d", pdrHighPriority->pdr_id.rule_id);
  ...
  // LOG_DBG("Add session");
  Logger::upf_app().debug("Add session");
...
void SessionManager::removeBPFSession(uint64_t seid)
{
  ...
  if(mSeidToSession.find(seid) == mSeidToSession.end()){
    ...
    Logger::upf_app().error("The session %d does not exist. Cannot be removed", seid);
...
void SessionManager::removeSession(uint64_t seid)
{
  ...
  // LOG_DBG("Session {} has been removed", seid);
  Logger::upf_app().info("Session %d has been removed", seid);
}
