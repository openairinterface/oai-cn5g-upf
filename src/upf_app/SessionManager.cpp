...
void SessionManager::createBPFSession(
  ...
  // LOG_DBG("Session {} received... Lets prepare the UP",
  // pSession->get_up_seid());
  Logger::upf_app().debug(
      "Session %d received... Lets prepare the UP", pSession->get_up_seid());
...
void SessionManager::removeBPFSession(uint64_t seid) {
  if (mSeidToSession.find(seid) == mSeidToSession.end()) {
    ...
    Logger::upf_app().error(
        "The session %d does not exist. Cannot be removed", seid);
