 ...
 * Date:        2026-03
 * Changes:     New file. CRTP base template for all XDP pipeline stage
 *              programs. Provides:
 *                - skeleton_ / lifecycle_ members
 *                - Load() / Attach() lifecycle actions
 *                - GetBpfObject()  -- for bpf_map__reuse_fd sharing
 *                - GetMaps()       -- access to all maps container
 *                - GetMap(name)    -- generic BPFMap getter by name
 *                - ConfigureMapSize(name, size) -- pre-load map sizing
 *                  via bpf_object__find_map_by_name (generic across all
 *                  skeleton types since skeleton_->obj is always bpf_object*)
 *                - InitMaps()      -- wraps FDs after load
 *              What stays in subclass:
 *                - GetXdpProgram() -- skeleton_->progs.xxx is type-specific
 *                - rodata configuration (rodata struct is type-specific)
 *                - Typed map getters as thin wrappers over GetMap()
 */
// clang-format on
 ...
 * @date 2026-03
 *
 * Eliminates repetitive boilerplate across all 8 stage programs.
 *
 * Usage
 * -----
 * @code
 * class URRProgram : public XDPStageProgram<xdp_urr_apply_kern_c> {
 * public:
 *   URRProgram() {
 *     InitLifecycle(
 *       [this]() -> xdp_urr_apply_kern_c* {
 *         auto* s = xdp_urr_apply_kern_c__open();
 *         ConfigureMapSize("urr_config_map",   upf::GetMaxPduSessions());
 *         ConfigureMapSize("urr_volume_counters_map",
 * upf::GetMaxPduSessions()); return s;
 *       },
 *       xdp_urr_apply_kern_c__load,
 *       xdp_urr_apply_kern_c__attach,
 *       xdp_urr_apply_kern_c__destroy);
 *   }
 *   struct bpf_program* GetXdpProgram() const {
 *     return skeleton_ ? skeleton_->progs.urr_apply : nullptr;
 *   }
 *   std::shared_ptr<BPFMap> GetUrrConfigMap()  const { return
 * GetMap("urr_config_map");  } std::shared_ptr<BPFMap> GetUrrVolumeMap()  const
 * { return GetMap("urr_volume_counters_map"); }
 * };
 * @endcode
 */
