 ...
 * Date:        2026-03
 * Changes:     New file. Mirrors session_lookup_ip_user.h exactly in
 *              structure. ETH PDU path counterpart.
 *              xdp_session_lookup_eth_kern.c includes:
 *                eth_pdu_maps.h         -> session_by_mac_map       (runtime)
 *                                          eth_session_mapping_map  (runtime)
 *                                          eth_session_pdrs_map     (runtime)
 *                                          eth_rules_match_pdr_map  (runtime)
 *                                          eth_egress_ifindex_map   (runtime)
 *                                          mac_pdu_session_map      (runtime)
 *                tail_call_dispatcher.h -> tail_call_progs_map      (shared)
 *                                          packet_context_map       (shared)
 *                                          session_rules_enabled_map(shared)
 *              rodata: MAX_UPF_INTERFACES.
...
/**
 * @file  session_lookup_eth_user.h
 * @brief User-space lifecycle manager for the xdp_session_lookup_eth XDP
 *        program.
 *
 * Responsibilities:
 *   - Open the xdp_session_lookup_eth_kern_c skeleton and configure its maps.
 ...
 *
 * Maps owned (from xdp_session_lookup_eth_kern.c includes):
 *   eth_pdu_maps.h:
 *     - session_by_mac_map       (runtime: MAX_PDU_SESSIONS)
 *     - eth_session_mapping_map  (runtime: MAX_PDU_SESSIONS)
 *     - eth_session_pdrs_map     (runtime: MAX_PDU_SESSIONS)
 *     - eth_rules_match_pdr_map  (runtime: MAX_PDU_SESSIONS x MAX_PDRS)
 *     - eth_egress_ifindex_map   (runtime: MAX_UPF_INTERFACES)
 *     - mac_pdu_session_map      (runtime: MAX_PDU_SESSIONS)
 *   tail_call_dispatcher.h (shared from primary via reuse_fd):
 *     - tail_call_progs_map       (fixed)
 ...
 *       tail call from xdp_n3_eth_entry or xdp_n6_eth_entry.
 */
