...
void QERProgram::Setup(
  ...
  if (!qers.empty()) {
    ...
    // NOTE: TC class handles are not globally allocated - they are derived from
    // the SEID. The PDU-session parent class is the low 16 bits of the SEID
    // (casted_seid below) and each QoS-flow class is generate_minor_id(seid,
    // qfi) (see below). All of these share the single "1:" major namespace on
    // the N3 interface, so the derived minor IDs must be globally unique, yet:
    //   - casted_seid collides once two live SEIDs share their low 16 bits
