 ...
 * Date:        2026-03
 * Changes:     Rewrote constructor -- uses own lifecycle_ (BPFProgram pattern).
 *              Map sizing via bpf_object__find_map_by_name before load.
 *              Added Load(), GetBpfObject(), GetXdpProgram().
 *              SetMaps() receives map refs from UPF_XDPProgram::InitializeMaps().
 *              All session lifecycle methods preserved unchanged. No upf_cfg.
 * 3GPP Refs:   3GPP TS 29.244 V17.10.0 ss8.2.54 URR ID
 */
// clang-format on
...
URRProgram::URRProgram() : BPFProgram() {
  auto open_fn = [this]() -> xdp_urr_apply_kern_c* {
    ...
    if (!s) throw std::runtime_error("Failed to open xdp_urr_apply skeleton");
    ...
    set_size("urr_config_map", upf::GetMaxPduSessions());
    set_size("urr_volume_counters_map", upf::GetMaxPduSessions());
    if (s->rodata) s->rodata->MAX_PDU_SESSIONS = upf::GetMaxPduSessions();
    return s;
  };
  lifecycle_ = std::make_shared<UrrProgramLifeCycle>(
      open_fn, xdp_urr_apply_kern_c__load, xdp_urr_apply_kern_c__attach,
      xdp_urr_apply_kern_c__destroy);
  skeleton_ = lifecycle_->open();
  Logger::upf_app().debug("URRProgram: skeleton opened");
}

//------------------------------------------------------------------------------
void URRProgram::Load() {
  lifecycle_->load();
  auto maps       = std::make_shared<BPFMaps>(skeleton_->skeleton);
  urr_config_map_ = std::make_shared<BPFMap>(maps->GetMap("urr_config_map"));
  urr_volume_counters_map_ =
      std::make_shared<BPFMap>(maps->GetMap("urr_volume_counters_map"));
  Logger::upf_app().debug("URRProgram: skeleton loaded");
}
...
//------------------------------------------------------------------------------
void URRProgram::SetMaps(
    std::shared_ptr<BPFMap> urr_config_map,
    std::shared_ptr<BPFMap> urr_volume_counters_map) {
  if (!urr_config_map || !urr_volume_counters_map)
    throw std::invalid_argument("URRProgram::SetMaps: null BPFMap pointer");
  urr_config_map_          = std::move(urr_config_map);
  urr_volume_counters_map_ = std::move(urr_volume_counters_map);
  Logger::upf_app().debug("URRProgram: maps set");
}
