...
# The script checks only tracked files, as it uses git ls-files. Untracked files
# are reported separately and must be added with git add before they are scanned.
#
...
VERBOSE=0
if [[ $# -gt 1 ]]; then
  echo "check-licenses: expected at most one argument, got $#" >&2
...
if [[ $untracked -gt 0 ]]; then
  echo "WARNING: $untracked file(s) are untracked and were not scanned"
  echo "      add them with git add before running the license check"
  if [[ $VERBOSE -eq 1 ]]; then
    printf 'WARNING: untracked file was not scanned: %s\n' "${UNTRACKED_FILES[@]}"
  fi
  echo
fi
