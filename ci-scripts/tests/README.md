...
class-id arithmetic, log strings, deployment settings, harness wiring, and **all 16
scenarios**. [`unit/test_boundary.py`](unit/test_boundary.py) checks that it actually
depends on `pfcpkit` rather than reimplementing it.

Testing another UPF means writing your own adapter and harness and adapting the
scenarios — see pfcp-kit's
[`README.md`](https://github.com/openairinterface/pfcp-kit/blob/main/README.md), which
is the authoring guide, and
[`pfcpkit/examples/`](https://github.com/openairinterface/pfcp-kit/tree/main/pfcpkit/examples)
for a version that runs with no UPF at all.

**Prefer a picture?** pfcp-kit's
[`docs/ARCHITECTURE.md`](https://github.com/openairinterface/pfcp-kit/blob/main/docs/ARCHITECTURE.md)
walks through a five-page diagram with the images inline and pointers into the code —
pages 1–3 and 5 describe the library, page 4 covers extending it or adding a scenario
here.
...
environment with its preflight. 62 unit tests here run with no UPF and no root (pfcpkit
carries its own, in its own repo).
...
pfcp-kit's architecture doc, page 2, traces each assertion down to the shell command
that answers it — start there if you want to check a verdict by hand.
...
.venv/bin/python -m pytest unit/oai -q
...
Worth running the first after any change to `pfcpkit`'s `inspect/` (in its own repo) or
to `upf_test/` here: a silently wrong offset makes an *absence* assertion pass for the
wrong reason, which is the worst failure a suite can have.
...
file changes. Pfcp-kit's architecture doc, page 4, has the same thing as a picture.
...
Only a genuinely new PFCP IE requires touching shared code: add a field to `models.py`
and a builder in `pfcp/codec.py`, in [pfcp-kit](https://github.com/openairinterface/pfcp-kit).

## Layout

`upf_test/` holds everything true only of this UPF, including all 16 scenarios; it
knows nothing about any other UPF and depends on `pfcpkit`, an external package. Pfcp-kit's
architecture doc, page 1, shows how the two relate and page 5 lists every module and what
it exposes.
...
[`unit/test_boundary.py`](unit/test_boundary.py) checks that `upf_test` actually depends
on `pfcpkit` rather than reimplementing it. `bpftool` and `tc` run **inside** the UPF
